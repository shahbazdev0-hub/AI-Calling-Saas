# # backend/app/api/v1/sms_chat.py - WITH APPOINTMENT BOOKING
...
#     else:
#         base_date = datetime.utcnow() + timedelta(days=1)  # Default to tomorrow
    
#     # Extract time (look for patterns like "2pm", "14:00", "2:30 pm")
...
#         appointment_datetime = base_date.replace(hour=10, minute=0, second=0, microsecond=0)
...
#         messages_cursor = db.sms_logs.find({
#             "user_id": user_id,
#             "$or": [
...
#         sms_logs = await messages_cursor.to_list(length=None)
...
#                 "role": role,
#                 "content": log["message"],
#                 "timestamp": log["created_at"].isoformat() if isinstance(log["created_at"], datetime) else log["created_at"],
#                 "status": log.get("status", "delivered"),
#                 "source": log.get("response_source", "manual")
#             })
            
#             if not customer_info and log["direction"] == "inbound":
#                 customer_info = {
...
#                     If customer wants to book an appointment, encourage them to say "book appointment"."""
#                 )
...
#                 ai_message = ai_response.get("response", "I apologize, but I'm unable to respond.")
#                 source = "openai"
#                 campaign_matched = False
#                 workflow_info = None
...
#             "direction": "inbound",
#             "from_number": phone_number,
#             "to_number": current_user.get("phone_number", "system"),
...
#         user_msg_result = await db.sms_logs.insert_one(user_msg_doc)
#         user_msg_id = str(user_msg_result.inserted_id)
        
#         ai_msg_doc = {
#             "user_id": user_id,
#             "direction": "outbound",
#             "from_number": current_user.get("phone_number", "system"),
#             "to_number": phone_number,
#             "message": ai_message,
#             "status": "sent",
#             "created_at": datetime.utcnow(),
...
#             "reply_to_sms_id": user_msg_id
#         }
        
#         if workflow_info:
#             ai_msg_doc["workflow_info"] = workflow_info
        
#         ai_msg_result = await db.sms_logs.insert_one(ai_msg_doc)
#         ai_msg_id = str(ai_msg_result.inserted_id)
        
#         logger.info(f"✅ Saved. Source: {source.upper()}\n")

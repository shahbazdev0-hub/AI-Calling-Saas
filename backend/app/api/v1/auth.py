...
# @router.post("/register", response_model=Token)  # ✅ CHANGED: Now returns Token
...
#         if len(user_data.password) > 100:  # Reasonable limit
#             raise HTTPException(
#                 status_code=status.HTTP_400_BAD_REQUEST,
#                 detail="Password is too long. Maximum 100 characters allowed."
#             )
        
#         # ✅ ADDED: Validate company name length
...
#         # ✅ NEW: Auto-login the user by creating access token
...
#         # ✅ CHANGED: Return token and user data (same format as login)
#         return {
#             "access_token": access_token,
#             "token_type": "bearer",
#             "expires_in": settings.ACCESS_TOKEN_EXPIRE_MINUTES * 60,
#             "user": format_user_response(user.dict())
#         }
...
# @router.post("/signup", response_model=Token)  # ✅ CHANGED: Now returns Token
...
@router.get("/me", response_model=UserResponse)
async def get_current_user_info(
    current_user: dict = Depends(get_current_user)  # ✅ FIXED: Changed from UserInDB to dict
):
    """Get current user info - FIXED to handle dict instead of UserInDB object"""
    try:
        ...
        if isinstance(current_user, dict):
            ...
            # If it's an object with .email attribute
            print(f"Getting user info for: {current_user.email}")
